...
class CheckHM{ 
    ...
    static public function mac(string $mac){
        return preg_match('/^[a-fA-F0-9]{12}$/', $mac) || 
		preg_match('/^[a-fA-F0-9]{2}(:[a-fA-F0-9]{2}){5}$/', $mac) ||
		preg_match('/^[a-fA-F0-9]{2}(\.[a-fA-F0-9]{2}){5}$/', $mac);
    }
    // 检查加密狗地址:00.00.00.00.00.00.00.00
    static public function dog(string $dog){
        return preg_match('/^[a-fA-F0-9]{2}(\.[a-fA-F0-9]{2}){7}$/', $dog) || preg_match('/^[a-fA-F0-9]{16}$/', $dog);
    }
